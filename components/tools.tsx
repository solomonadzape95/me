...
export default function Tools() {
    return (
       ...
       <span className="flex items-center justify-between gap-2"><h1 className="text-3xl lg:text-4xl">tools</h1> </span>
...
return   <span className="col-span-3 mb-2 w-full h-[15vh] border-2 border-gray-50/20 cursor-default hover:transition-all hover:duration-300 grid place-items-center overflow-hidden group grid place-items-center">
...
{content.map(db => <p key={db} className="text-xl xl:text-2xl max-w-11/12">{db}</p>)}
</div>
</span>
</span>
}
function VerticalCard({title, content } : {title: string, content: string[]}){
    ...
    {content.map(item => <p key={item} className="text-xl xl:text-2xl max-w-11/12">{item}</p>)}
    </span></span>
    }